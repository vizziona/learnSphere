...
import { Plus, Edit, Trash2, RefreshCw, Flask } from "lucide-react";
...
function ExperimentManager() {
  ...
  const fetchExperiments = async () => {
    try {
      ...
      toast.error(err.response?.data?.message || "Error fetching experiments");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      if (formData.id) {
        await axios.put(
          `http://localhost:5000/api/experiments/${formData.id}`,
          formData,
          {
            headers: {
        ...
        await axios.post("http://localhost:5000/api/experiments", formData, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });
      ...
      toast.error(err.response?.data?.message || "Error saving experiment");
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.error(err.response?.data?.message || "Error deleting experiment");
    }
  };
        ...
        <div style={styles.main}>
          <div style={styles.maxWidth}>
            <div style={styles.loading}>
              ...
              <p style={styles.loadingText}>Loading experiments...</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.main}>
        <div style={styles.maxWidth}>
          <h2 style={styles.title}>Manage Experiments</h2>
          <button
            ...
            onMouseDown={(e) => {
              e.target.style.transform = styles.addButtonActive.transform;
              e.target.style.boxShadow = styles.addButtonActive.boxShadow;
            }}
            onMouseUp={(e) => {
              e.target.style.transform = styles.addButtonHover.transform;
              e.target.style.boxShadow = styles.addButtonHover.boxShadow;
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {experiments.map((exp) => (
                <tr key={exp.id}>
                  <td style={styles.tableCell}>{exp.id}</td>
                  <td style={styles.tableCell}>{exp.label}</td>
                  <td style={styles.tableCell}>{exp.icon}</td>
                  <td style={styles.tableCell}>{exp.x}</td>
                  <td style={styles.tableCell}>{exp.y}</td>
          ...
          {open && (
            <div style={styles.dialog}>
              <h2 style={styles.dialogTitle}>
                {formData.id ? "Edit Experiment" : "Add Experiment"}
              </h2>
