...
const PhysicsNavigation = () => {
  ...
  const experimentCardsRef = useRef(experimentCards); // Ref to access latest experimentCards
  ...
  // Update experimentCardsRef whenever experimentCards changes
  useEffect(() => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/experiments?category=physics")
  ...
  const proceedToExperiment = () => {
    ...
    history.push({
      ...
      state: { selectedExperiment: `physics/${activePopup}` },
    });
  };
    ...
    navigationHelper.init({
      ...
      onPointerDown: (x, y) => {
        ...
        if (activePopup) {
          ...
          // Check for proceed button click
...
export default PhysicsNavigation;
