...
const FunNavigation = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/experiments?category=fun")
  ...
  const popupStyle = {
    ...
    padding: "20px",
    zIndex: 5,
    boxSizing: "border-box", // Corrected typo from boxBoxing
    display: "flex",
    flexDirection: "column",
    pointerEvents: "none",
  };
  ...
  return (
    <>
        ...
        {!activePopup &&
          experimentCards.map((card) => (
            <div
              ...
              onMouseLeave={(e) => {
                e.target.style.transform = isSelfieCam ? "scaleX(-1)" : "";
              }}
              onClick={() => {
      ...
      {!showInstruction && (
        <FlipCamButton
          buttonRef={changeCameraButtonRef}
